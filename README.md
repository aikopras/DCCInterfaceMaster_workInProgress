...
This mode is optimized for direct use with several **H-bridges**. Because both the normal and inverted DCC rail signals are provided, the H-bridges can be driven directly without the need for external logic components.
...
For **RP2040/RP2350** drivers, the three output signals are generated by the PIO (hardware), which results in a jitter free high quality DCC signal, with little CPU load. This processor is therefore recommended for new designs.
